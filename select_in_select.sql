...
--5
SELECT name, CONCAT(ROUND((SELECT population FROM world WHERE name='Germany')/population), '%') AS pe FROM world
...
WHERE population > ALL(SELECT 3*population FROM world y WHERE x.name <> y.name AND x.continent = y.continent);
